...
async function getCoordsAlt(css_selector) {
  ...
  try {
    ...
    client = await CDP();
    // extract domains
    var { Page, Runtime, DOM } = client;
    // enable events then start!
    await Promise.all([Page.enable(), Runtime.enable(), DOM.enable()]);

    var {root: {nodeId: documentNodeId}} = await DOM.getDocument();

    console.log(documentNodeId)

    var result = await DOM.querySelector({
    ...
    var retval = await DOM.getBoxModel({"nodeId": result.nodeId});
    ...
    var coords = {"x": center_x, "y": center_y, "node_id": result.nodeId, "root_node": documentNodeId};
...
async function getCoordsIframe(css_selector, iframe) {
  ...
  try {
    ...
    var screenPos = await Runtime.evaluate({
...
exports.getCoords = async function (css_selector) {
  let client;
  try {
    // connect to endpoint
    client = await CDP();
...
exports.getCoordsMulti = async function (css_selector, element_number) {
  let client;
  console.log('Getting coordinates for CSS selector: ', css_selector);
  try {
    // connect to endpoint
    client = await CDP();
    // extract domains
    let { Page, Runtime, DOM } = client;
    // enable events then start!
    await Promise.all([Runtime.enable()]);

    let result = null;
    let clientRectCmd = `var targetCoordEl = document.querySelectorAll('${css_selector}')['${element_number}']; if (targetCoordEl) { JSON.stringify(targetCoordEl.getClientRects()); }`;

    result = await Runtime.evaluate({expression: clientRectCmd,});

    // get offset screen positioning
    let screenPos = await Runtime.evaluate({
      expression: "JSON.stringify({offsetY: window.screen.height - window.innerHeight, offsetX: window.screen.width - window.innerWidth})"
    });
    ...
    console.log(`css_selector: ${css_selector} - element_number #${element_number}`, JSON.stringify(retVal));
    return new Promise((resolve, reject) => {
                resolve(retVal);
            })
  } catch (err) {
...
exports.getMoreResultsCoords = async function () {
  let client;
  try {
    // connect to endpoint
    client = await CDP();
    ...
    let result = null;
    let clientRectCmd = `var targetCoordEl; var elements = document.getElementsByTagName('span'); for (var i = 0; i < elements.length; i++) { if (elements[i].innerText === 'More results') { targetCoordEl = elements[i]; break; } } if (targetCoordEl) { JSON.stringify(targetCoordEl.getClientRects()); }`;
    ...
    console.log("More results coordinates:", JSON.stringify(retVal));
    return new Promise((resolve, reject) => {
                resolve(retVal);
            })
  } catch (err) {
